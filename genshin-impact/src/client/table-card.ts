 ...
 * hook and expands them out of the message column. In the genshin bubble that
...
export const GENSHIN_TABLE_SELECTOR = '.md-table-wide'
export const GENSHIN_TABLE_FILL_SELECTOR = '.md-table-wide'
const SKIN_OWNER = 'genshin-impact'
const EXPANDABLE_ATTRIBUTE = 'data-genshin-table-expandable'
const OPEN_ATTRIBUTE = 'data-genshin-table-open'
const CONTROL_ATTRIBUTE = 'data-genshin-table-expand'
const FRAME_ATTRIBUTE = 'data-genshin-table-frame'
const SCROLL_SUPPRESSED_ATTRIBUTE = 'data-genshin-table-scroll-suppressed'
const OVERLAY_ATTRIBUTE = 'data-genshin-table-lightbox'
 ...
 */
export function installGenshinTableCards(_ctx: Context): TableCardRuntime {
  const owner = Symbol('genshin-table-card-activation')
  ...
  const closeOverlay = (immediate = false): void => {
    ...
    root.dataset.genshinTableClosing = ''
  ...
  const openOverlay = (wrapper: HTMLElement): void => {
    ...
    backdrop.dataset.genshinTableBackdrop = ''

    const panel = document.createElement('div')
    panel.dataset.genshinTablePanel = ''
    ...
    panel.style.setProperty('--genshin-table-expanded-width', `${targetWidth}px`)

    const close = document.createElement('button')
    close.type = 'button'
    close.dataset.genshinTableClose = ''
    close.setAttribute('aria-label', '关闭展开表格')

    const scroller = document.createElement('div')
    scroller.dataset.genshinTableExpandedScroller = ''
    ...
    clone.dataset.genshinTableExpanded = ''
    ...
    const onClick = (event: MouseEvent): void => {
      ...
      if (target instanceof Element
        && (target.closest('[data-genshin-table-close]') !== null || target.hasAttribute('data-genshin-table-backdrop'))) {
        closeOverlay()
      }
    }
  ...
  try {
    ...
    observer = new MutationObserver((records) => {
      ...
      for (const record of records) {
        if (record.target instanceof HTMLElement) {
          const wrapper = record.target.matches(GENSHIN_TABLE_SELECTOR)
            ? record.target
            : record.target.closest<HTMLElement>(GENSHIN_TABLE_SELECTOR)
        ...
        for (const node of record.addedNodes) {
          ...
          if (node.matches(GENSHIN_TABLE_SELECTOR)) track(node as HTMLElement)
          else if (node.querySelectorAll(GENSHIN_TABLE_SELECTOR).length > 0) {
            node.querySelectorAll<HTMLElement>(GENSHIN_TABLE_SELECTOR).forEach(track)
          }
        }
        for (const node of record.removedNodes) {
          if (!(node instanceof Element)) continue
          if (node.matches(GENSHIN_TABLE_SELECTOR)) removed.add(node as HTMLElement)
          node.querySelectorAll<HTMLElement>(GENSHIN_TABLE_SELECTOR).forEach(wrapper => removed.add(wrapper))
        }
      }
    ...
    document.querySelectorAll<HTMLElement>(GENSHIN_TABLE_SELECTOR).forEach(track)
    return runtime
  } catch (error) {
    runtime.dispose()
    throw error
