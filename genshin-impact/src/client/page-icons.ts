...
export function installGenshinPageIcons(ctx: Context): void {
  ctx.effect(() => {
    const doc = document
  ...
  }, 'ui-skin-genshin-impact: page icons')
}
...
function mountPageIcons(doc: Document): () => void {
  ...
  try {
    ...
    for (const node of doc.head.querySelectorAll<HTMLLinkElement>('link[rel~="icon"], link[rel="manifest"]')) {
      const anchor = doc.createComment('genshin-impact: host icon')
    ...
    const append = (rel: string, href: string, type: string): HTMLLinkElement => {
      ...
      node.dataset.skinOwner = 'genshin-impact'
